...
import {
  Box, Button, IconButton, Stack, Table, TableBody, TableCell, TableHead,
...
type SweepRow = LteAutomationRow
 ...
 * and the readout stays fixed, so the table fills the panel without every
 * input becoming the same width.
 */
const COLS = '20px minmax(140px, 1.5fr) minmax(140px, 1.5fr) minmax(100px, 1fr) 118px 32px'
const MIN_TABLE_W = 590

const DEFAULT_ROW: SweepRow = { earfcn: '', power: DEFAULT_POWER, marginDb: '1' }

/** Fill in a row read back from storage. Snapshots written before the
 *  tolerance moved onto the row hold only the channel and power. */
...
export function LteAutomationPanel({
  ...
  const importFile = async (f: File) => {
    try {
      const imported = parseLteCwCsv(await f.text())
  ...
  const addRow = () => {
    ...
    // the last one's power and tolerance: adding a channel to an existing
    // sweep is far more common than starting a different kind of measurement.
  ...
  const rowPoints = (r: SweepRow): PlanPoint[] => {
    ...
    if (channels.length === 0 || !marginValid(r)) return []
  ...
  const anyInvalid = rows.some((r) => r.earfcn.trim() !== '' && !marginValid(r))
          ...
          action={
            <Stack direction="row" alignItems="center" spacing={1.25}>
              ...
              {/* Settle is the one thing here that is genuinely run-wide: it is
                  about instrument timing, not about the channel, so it stays
                  out of the table. */}
